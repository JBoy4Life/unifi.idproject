...
public class DefaultAccessManager implements AccessManager<OperatorSessionData> {
    ...
    public DefaultAccessManager(DatabaseProvider dbProvider) {
        ...
        this.permissionCache = CacheBuilder.newBuilder()
                ...
                .build(CacheLoader.from(op -> db.execute(sql ->
    ...
    public void updateOperationList(Collection<ServiceRegistry.Operation> permissionedOperations) {
        ...
        var operationNames = this.permissionedOperations.keySet().stream()
                .flatMap(o -> Stream.of(getWildcardOperations(o)))
    ...
    public boolean authorize(String operationName, OperatorSessionData session) {
        ...
        return operation == null || operation.access == Access.PERMISSIONED_NOT_CHECKED
                || (operator != null && containsOperation(getPermissions(operator), operationName));
    }
    ...
    public boolean isAllowed(String operationName, OperatorPK operator) {
        ...
        return containsOperation(getPermissions(operator), operationName);
    }
    ...
    private static boolean containsOperation(Set<String> permittedOperations, String operation) {
        return Arrays.stream(getWildcardOperations(operation)).anyMatch(permittedOperations::contains);
    }

    private static String[] getWildcardOperations(String operationName) {
        var parts = operationPartDelimiter.split(operationName);
        return new String[]{ operationName, parts[0] + "." + parts[1] + ".*", parts[0] + ".*.*" };
    }
}
